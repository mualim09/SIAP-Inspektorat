<?php
	...
	preg_match_all('/(?P<izin>(?<=izin:)(.*))/', $spt->jenisSpt->dasar, $matches);
	//dd($matches['izin']);
	$dasar = '';
	
	if( isset(($matches['izin'])) && count($matches['izin'])>1){
		foreach(($matches['izin']) as $dasar2){
			$i = $i+1;
				$dasar .= '<tr>';
		...
		$dasar .= '<tr>';
		$dasar .= "<td align=\"left\" valign=\"top\" colspan=\"2\" style=\"text-align:justify\" >".$matches['izin'][0]."</td>";
		$dasar .= "</tr>";
	}
				...
				<table width="100%">
					<tr>
						...
						<td style="text-align:justify">
							<p>Untuk melakukan pemeriksaan guna menyelesaikan Permohonan Izin melakukan  Perceraian 
								@if($spt->tambahan !== null)
								{{$spt->tambahan}}
								@endif
							</p>
							<p>
								Jangka waktu pemeriksaan selama {{ $spt->lama_hari }} kerja pada periode tanggal {{$spt->periode}}.
							</p>
